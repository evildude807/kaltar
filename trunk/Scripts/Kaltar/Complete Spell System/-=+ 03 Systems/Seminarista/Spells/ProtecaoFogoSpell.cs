...
{
	public class ProtecaoFogoSpell : SeminaristaSpell {
		...
		public ProtecaoFogoSpell( Mobile caster, Item scroll ) : base( caster, scroll, m_Info ) {
		}
		...
		public void Target( Mobile m ) {
			...
			if ( sobreEfeito( m ) ){
				Caster.LocalOverheadMessage( MessageType.Regular, 0x481, false, "O alvo já esta sobre o efeito de magia Proteção Fogo." );
			}
                ...
                resistencias[0] = new ResistanceMod(ResistanceType.Fire, protecao);
				...
				m.SendMessage( "Você esta sobre o efeito da magia Proteção Fogo" );
		...
		private class InternalTarget : Target {
			private ProtecaoFogoSpell m_Owner;

			public InternalTarget( ProtecaoFogoSpell owner ) : base( 1, false, TargetFlags.Beneficial ) {
				m_Owner = owner;
			}
		...
		private class InternalTimer : Timer {
            ...
            public InternalTimer(Mobile m, ResistanceMod[] resistencias, TimeSpan delay ) : base( delay ) {
				...
				bf = new BuffInfo( BuffIcon.Bless, 1075843, new TextDefinition("Proteção Fogo"));
