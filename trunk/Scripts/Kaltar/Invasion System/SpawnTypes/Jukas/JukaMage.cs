...
namespace Scripts.Invasion_System
{
    ...
    public class InvasionJukaMage : InvasionSpawn
	{
		[Constructable]
		...
		public override void OnThink()
		{
			if ( DateTime.Now >= m_NextAbilityTime )
			{
				...
				{
					if ( m is JukaLord && IsFriend( m ) && m.Combatant != null && CanBeBeneficial( m ) && m.CanBeginAction( typeof( InvasionJukaMage ) ) && InLOS( m ) )
					{
						toBuff = (JukaLord)m;
						break;
					}
				...
				if ( toBuff != null )
				{
					if ( CanBeBeneficial( toBuff ) && toBuff.BeginAction( typeof( InvasionJukaMage ) ) )
					{
		...
		private void Unbuff( object state )
		{
			...
			toDebuff.EndAction( typeof( InvasionJukaMage ) );
		...
		public InvasionJukaMage( Serial serial ) : base( serial )
		{
		}
