...
namespace Server.Items
{
	public class SkillStoneGump : Gump
	{
		...
		public SkillStoneGump( SkillBuyStone stone, Mobile from, int page ) : base( 180, 30 )
		{
			...
			AddLabel( 434, 132, 43, "Token Cost per " + m_Stone.SkillIncrease + "%:");
			AddLabel( 556, 132, 62, m_Stone.PriceInToken.ToString());
			...
			for( int i = 0; i < m_From.Skills.Length && i < 20; i++ )
			{
				...
				AddButton( (i >= 10 ? 637 : 319), (i >= 10 ? 225+((i-10)*30) : 225+i*30), 4014, 4016, 200+i+iStart, GumpButtonType.Reply, 0 );
			}
		}
		...
		public override void OnResponse( NetState sender, RelayInfo info )
		{
			...
			else if( BID >= 100 && BID <= m_From.Skills.Length+100 )
			{
				...
				else if( m_From.Skills.Cap + m_Stone.SkillIncrease > m_From.SkillsCap )
				...
				else
				{
						...
						(Banker.Withdraw( m_From, m_Stone.PriceInGold )) )
					{
						...
						m_From.SendMessage( "You payed {0} Gold to gain {1} skill in {2}", m_Stone.PriceInGold, m_Stone.SkillIncrease, m_From.Skills[(SkillName)skill].Name );
					}
					else
			...
			else if( BID >= 200 && BID <= m_From.Skills.Length+200 )
			{
				int skill = BID - 200;
				if( skill < 0 || skill >= m_From.Skills.Length )
					return;
				...
				{
					Container pack = m_From.Backpack;
					if( pack != null && pack.ConsumeTotal( typeof( Gold ), m_Stone.PriceInToken ) )
					{
						m_From.Skills[(SkillName)skill].Base += m_Stone.SkillIncrease;
						m_From.SendMessage( "You payed {0} Tokens to gain {1} skill in {2}", m_Stone.PriceInToken, m_Stone.SkillIncrease, m_From.Skills[(SkillName)skill].Name );
					}
					else
						m_From.SendMessage( "You need {0} Tokens to use this!", m_Stone.PriceInToken );
				}
