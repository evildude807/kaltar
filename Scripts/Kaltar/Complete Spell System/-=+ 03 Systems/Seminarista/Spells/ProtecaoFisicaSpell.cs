...
{
	public class ProtecaoFisicaSpell : SeminaristaSpell {
		...
		public ProtecaoFisicaSpell( Mobile caster, Item scroll ) : base( caster, scroll, m_Info ) {
		}
		...
		public void Target( Mobile m ) {
			...
			if ( sobreEfeito( m ) ){
                Caster.LocalOverheadMessage(MessageType.Regular, 0x481, false, "O alvo já esta sobre o efeito de magia Proteção Física.");
			}
				...
				m.SendMessage( "Você esta sobre o efeito da magia Proteção Física" );
		...
		private class InternalTarget : Target {
			private ProtecaoFisicaSpell m_Owner;

			public InternalTarget( ProtecaoFisicaSpell owner ) : base( 1, false, TargetFlags.Beneficial ) {
				m_Owner = owner;
			}
		...
		private class InternalTimer : Timer {
            ...
            public InternalTimer(Mobile m, ResistanceMod[] resistencias, TimeSpan delay ) : base( delay ) {
				...
				bf = new BuffInfo( BuffIcon.Bless, 1075843, new TextDefinition("Proteção Física"));
			...
			public void DoExpire() {
				...
				removerEfeito( alvo );
                alvo.SendMessage("O efeito da magia Proteção Física acabou.");
				Stop();
			}
